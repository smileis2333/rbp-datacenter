...
public class ChannelServiceImpl implements ChannelService {
    ...
    public ChannelRespDto uploadingChannel(OnlinePlatform onlinePlatform) {
        ...
        if (warehouseChannelRangeList != null && warehouseChannelRangeList.size() > 0) {
            ...
            List<InnoChannelDto> reqList = new ArrayList<>();
            for (Channel item : channelList) {
                InnoChannelDto innoChannelDto = new InnoChannelDto(item.getId().toString(), item.getName(), item.getLinkManMobile(), item.getLinkMan(), item.getCode(),
                        item.getAddress(), "", item.toString(),  item.getCode(), new BigDecimal(0), new BigDecimal(0));
                reqList.add(innoChannelDto);
            }
    ...
    public ChannelRespDto uploadingWarehouse(OnlinePlatform onlinePlatform) {
        ...
        if (warehouseChannelRangeList != null && warehouseChannelRangeList.size() > 0) {
            ...
            List<Channel> channelList = channelDao.selectList(queryWrapper);
            List<InnoWarehouseDto> innoWarehouseDtoList = new ArrayList<>();
            Integer i = 1;
            for (Channel item : channelList) {
                InnoWarehouseDto warehouse = new InnoWarehouseDto(i.toString(), item.getRegion(), item.getCode(), item.getLinkMan(), item.getName(), item.getLinkManMobile(), item.getAddress());
                innoWarehouseDtoList.add(warehouse);
                i++;
            }
            ...
            warehouseReqDto.setData(innoWarehouseDtoList);
    ...
    public ChannelRespDto uploadingCloudWarehouse(OnlinePlatform onlinePlatform) {
        ...
        List<InnoWarehouseDto> innoWarehouseDtoList = new ArrayList<>();
        InnoWarehouseDto warehouseItem = new InnoWarehouseDto("1", "", warehouse.getCode(), "", warehouse.getName(), "", "");
        innoWarehouseDtoList.add(warehouseItem);
